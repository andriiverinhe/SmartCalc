...
void s21_getNumber(const char *input, char *num) {
  size_t length = strcspn(input, "+-*/^%%() cstal");
...
s21_stack *s21_getStackOnPolishNotation(s21_stack *main) {
  ...
  while (main != NULL) {
    ...
    if (main->priority == 1 || main->priority == 2 || main->priority == 3 || main->priority == 4) {
      if (!sign)
...
double s21_NewCalculator(s21_stack *st) {
  ...
  while (tmp) {
    if (tmp->priority != NUMBER) {
      ...
      if (tmp->next && tmp->next->next && tmp->next->next && tmp->next &&
          tmp->next->priority == NUMBER &&
          tmp->next->next->priority == NUMBER && lex < 6) {
